...
object TaskManager {
    ...
    fun listTasks(showAll: Boolean = true) { // showAll can be used to filter later if needed
        if (tasks.isEmpty()) {
...
fun main() {
    val scanner = Scanner(System.`in`) // Using java.util.Scanner for simplicity
    ...
    while (true) {
        ...
        when (choice) {
            ...
            "6" -> {
                ...
                val id = scanner.nextLine()?.toIntOrNull()
                if (id != null) {
                    TaskManager.markTaskComplete(id, false) // Reusing with false
                } else {
                    println("Invalid ID format.")
                }
            }
